...
//!     let mut bv = BitVec::from_elem_in(&arena, max_prime, true);
//!
...
use super::{Vec, vec::FromIteratorIn};;
use crate::*;
...
/// use bit_vec::BitVec;
///
/// let mut bv = BitVec::from_elem_in(&arena, 10, false);
///
...
impl<'a, B: BitBlock> Index<usize> for BitVec<'a, B> {
    type Output = bool;
...
impl<'a> BitVec<'a, u32> {
    ...
    /// ```
    #[inline]
    pub fn from_elem_in(arena: &'a Arena, nbits: usize, bit: bool) -> Self {
        let nblocks = blocks_for_bits::<B>(nbits);
        let mut bit_vec = BitVec {
            storage: Vec::from_std_in(&arena, vec![if bit { !B::zero() } else { B::zero() }; nblocks]),
            nbits,
        };
    ...
    #[inline]
    pub fn with_capacity_in(&arena: &'a Arena, nbits: usize) -> Self {
        BitVec {
            storage: Vec::with_capacity_in(&arena, blocks_for_bits::<B>(nbits)),
            nbits: 0,
        }
    }
    ...
    where
        F: FnMut(usize) -> bool,
    {
        let mut bit_vec = BitVec::from_elem_in(&arena, len, false);
        for i in 0..len {
            bit_vec.set(i, f(i));
        }
        bit_vec
...
impl<'a, B: BitBlock> BitVec<'a, B> {
    ...
    /// last word.
    #[inline]
    fn process<F>(&mut self, other: &BitVec<'a, B>, mut op: F) -> bool
    where
        F: FnMut(B, B) -> B,
    {
    ...
    /// use bit_vec::BitVec;
    ///
    /// let mut bv = BitVec::from_elem_in(&arena, 5, false);
    /// bv.set(3, true);
    /// assert_eq!(bv[3], true);
    /// ```
    #[inline]
    ...
    /// use bit_vec::BitVec;
    ///
    /// let mut bv = BitVec::from_elem_in(&arena, 5, true);
    ...
    /// ```
    /// use bit_vec::BitVec;
    /// let mut a = BitVec::with_capacity_in(&arena, 100);
    /// a.push(true);
    /// a.push(false);
    /// a.push(false);
    /// a.push(true);
    ...
    /// use bit_vec::BitVec;
    ///
    /// let mut bv = BitVec::from_elem_in(&arena, 3, true);
    /// bv.set(1, false);
    ///
    /// assert_eq!(bv.to_bytes(), [0b10100000]);
    ///
    /// let mut bv = BitVec::from_elem_in(&arena, 9, false);
    /// bv.set(2, true);
    /// bv.set(8, true);
    ///
    /// assert_eq!(bv.to_bytes(), [0b00100000, 0b10000000]);
    /// ```
    pub fn to_bytes(&self) -> StdVec<u8> {
        self.ensure_invariant();
        // Oh lord, we're mapping this to bytes bit-by-bit!
        fn bit<B: BitBlock>(bit_vec: &BitVec<'_, B>, byte: usize, bit: usize) -> u8 {
    ...
    /// use bit_vec::BitVec;
    ///
    /// let mut bv = BitVec::from_elem_in(&arena, 3, false);
    /// bv.reserve(10);
    /// assert_eq!(bv.len(), 3);
    ...
    /// use bit_vec::BitVec;
    ///
    /// let mut bv = BitVec::with_capacity_in(&arena, 100);
    /// bv.reserve(10);
...
/*
impl<'a, B: BitBlock> FromIterator<bool> for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> Extend<bool> for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> PartialOrd for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> Ord for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> fmt::Debug for BitVec<'a, B> {
...
impl<'a, B: BitBlock> hash::Hash for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> cmp::PartialEq for BitVec<'a, B> {
    #[inline]
...
impl<'a, B: BitBlock> cmp::Eq for BitVec<'a, B> {}

impl<'a, T> FromIteratorIn<'a, T> for BitVec<'a, T> {
...
impl<'a, B: BitBlock> IntoIterator for &'a BitVec<'a, B> {
    type Item = bool;
...
pub struct IntoIter<'a, B = u32> {
    bit_vec: BitVec<'a, B>,
    range: Range<usize>,
}

impl<'a, B: BitBlock> Iterator for IntoIter<'a, B> {
    type Item = bool;
...
impl<'a, B: BitBlock> DoubleEndedIterator for IntoIter<'a, B> {
    #[inline]
    fn next_back(&mut self) -> Option<bool> {
        self.range.next_back().map(|i| self.bit_vec.get(i).unwrap())
    }
...
impl<'a, B: BitBlock> ExactSizeIterator for IntoIter<'a, B> {}

impl<'a, B: BitBlock> IntoIterator for BitVec<'a, B> {
    type Item = bool;
...
mod tests {
    use crate::*;
    use crate::collections::vec::CollectIn;
    ...
    fn test_to_str() {
        ...
        let zerolen = BitVec::with_capacity_in(&arena, 100);
        assert_eq!(format!("{:?}", zerolen), "");

        let eightbits = BitVec::from_elem_in(&arena, 8, false);
    ...
    fn test_0_elements() {
        ...
        let act = BitVec::with_capacity_in(&arena, 500);
        let exp = Vec::with_capacity_in(&arena, 500);
    ...
    fn test_1_element() {
        let arena = Arena::with_capacity(5_242_880);
        let mut act = BitVec::from_elem_in(&arena, 1, false);
        assert!(act.eq_vec(&[false]));
        assert!(act.none() && !act.all());
        act = BitVec::from_elem_in(&arena, 1, true);
    ...
    fn test_2_elements() {
        ...
        let mut b = BitVec::from_elem_in(&arena,2, false);
        b.set(0, true);
        b.set(1, false);
    ...
    fn test_10_elements() {
        ...
        act = BitVec::from_elem_in(&arena, 10, false);
        assert!(
        ...
        act = BitVec::from_elem_in(&arena, 10, true);
    ...
    fn test_31_elements() {
        ...
        act = BitVec::from_elem_in(&arena, 31, false);
        assert!(act.eq_vec(&[
        ...
        act = BitVec::from_elem_in(&arena, 31, true);
        assert!(act.eq_vec(&[
    ...
    fn test_32_elements() {
        ...
        act = BitVec::from_elem_in(&arena, 32, false);
        assert!(act.eq_vec(&[
            false, false, false, false, false, false, false, false, false, false, false, false,
            false, false, false, false, false, false, false, false, false, false, false, false,
        ...
        act = BitVec::from_elem_in(&arena, 32, true);
        assert!(act.eq_vec(&[
            true, true, true, true, true, true, true, true, true, true, true, true, true, true,
            true, true, true, true, true, true, true, true, true, true, true, true, true, true,
            true, true, true, true
    ...
    fn test_33_elements() {
        ...
        act = BitVec::from_elem_in(&arena, 33, false);
        assert!(act.eq_vec(&[
            false, false, false, false, false, false, false, false, false, false, false, false,
            false, false, false, false, false, false, false, false, false, false, false, false,
        ...
        act = BitVec::from_elem_in(&arena, 33, true);
        assert!(act.eq_vec(&[
            true, true, true, true, true, true, true, true, true, true, true, true, true, true,
            true, true, true, true, true, true, true, true, true, true, true, true, true, true,
    ...
    fn test_equal_differing_sizes() {
        ...
        let v0 = BitVec::from_elem_in(&arena, 10, false);
        let v1 = BitVec::from_elem_in(&arena, 11, false);
        assert_ne!(v0, v1);
    }
    ...
    fn test_equal_greatly_differing_sizes() {
        ...
        let v0 = BitVec::from_elem_in(&arena, 10, false);
        let v1 = BitVec::from_elem_in(&arena, 110, false);
        assert_ne!(v0, v1);
    }
    ...
    fn test_equal_sneaky_small() {
        ...
        let mut a = BitVec::from_elem_in(&arena, 1, false);
        a.set(0, true);

        let mut b = BitVec::from_elem_in(&arena, 1, true);
        b.set(0, true);
    ...
    fn test_equal_sneaky_big() {
        ...
        let mut a = BitVec::from_elem_in(&arena, 100, false);
        for i in 0..100 {
            a.set(i, true);
        }

        let mut b = BitVec::from_elem_in(&arena, 100, true);
        for i in 0..100 {
            b.set(i, true);
        }
    ...
    fn test_bit_vec_iterator() {
        ...
        let bit_vec: BitVec = bools.iter().map(|n| *n).collect_in(&arena);

        assert_eq!(bit_vec.iter().collect::<StdVec<bool>>(), bools);

        let long: StdVec<_> = (0..10000).map(|i| i % 2 == 0).collect();
        let bit_vec: BitVec = long.iter().map(|n| *n).collect_in(&arena);
        assert_eq!(bit_vec.iter().collect_in::<Vec<bool>>(&arena), long)
    }
    */
    ...
    fn test_small_difference() {
        let arena = Arena::with_capacity(5_242_880);
        let mut b1 = BitVec::from_elem_in(&arena, 3, false);
        let mut b2 = BitVec::from_elem_in(&arena, 3, false);
        b1.set(0, true);
        b1.set(1, true);
        b2.set(1, true);
        b2.set(2, true);
    ...
    fn test_big_difference() {
        let arena = Arena::with_capacity(5_242_880);
        let mut b1 = BitVec::from_elem_in(&arena, 100, false);
        let mut b2 = BitVec::from_elem_in(&arena, 100, false);
        b1.set(0, true);
        b1.set(40, true);
        b2.set(40, true);
        b2.set(80, true);
    ...
    fn test_small_clear() {
        let arena = Arena::with_capacity(5_242_880);
        let mut b = BitVec::from_elem_in(&arena, 14, true);
        assert!(!b.none() && b.all());
        b.clear();
        assert!(b.none() && !b.all());
    }
    ...
    fn test_big_clear() {
        let arena = Arena::with_capacity(5_242_880);
        let mut b = BitVec::from_elem_in(&arena, 140, true);
        assert!(!b.none() && b.all());
        b.clear();
        assert!(b.none() && !b.all());
    }
    ...
    fn test_bit_vec_lt() {
        let arena = Arena::with_capacity(5_242_880);
        let mut a = BitVec::from_elem_in(&arena, 5, false);
        let mut b = BitVec::from_elem_in(&arena, 5, false);
    ...
    fn test_bit_vec_push_pop() {
        ...
        let mut s = BitVec::from_elem_in(&arena, 5 * U32_BITS - 2, false);
    ...
    fn test_bit_vec_truncate() {
        let arena = Arena::with_capacity(5_242_880);
        let mut s = BitVec::from_elem_in(&arena, 5 * U32_BITS, true);

        assert_eq!(s, BitVec::from_elem_in(&arena, 5 * U32_BITS, true));
        assert_eq!(s.len(), 5 * U32_BITS);
        s.truncate(4 * U32_BITS);
        assert_eq!(s, BitVec::from_elem_in(&arena, 4 * U32_BITS, true));
        ...
        assert_eq!(s, BitVec::from_elem_in(&arena, 3 * U32_BITS - 10, true));
        assert_eq!(s.len(), 3 * U32_BITS - 10);
        s.truncate(0);
        assert_eq!(s, BitVec::from_elem_in(&arena, 0, true));
        assert_eq!(s.len(), 0);
    }
    ...
    fn test_bit_vec_append() {
        ...
        let mut b = BitVec::with_capacity_in(&arena, 100);
        b.push(false);
        b.push(true);
        b.push(true);
    ...
    fn test_into_iter() {
        ...
        let bit_vec: BitVec<'_, bool> = bools.iter().map(|n| *n).collect_in(&arena);
        let mut iter = bit_vec.into_iter();
        assert_eq!(Some(true), iter.next_back());
        assert_eq!(Some(true), iter.next());
        assert_eq!(Some(false), iter.next());
    ...
    fn iter() {
        let arena = Arena::with_capacity(5_242_880);
        let b = BitVec::with_capacity_in(&arena, 10);
        let _a: Iter = b.iter();
    }
    ...
    fn test_serialization() {
        let arena = Arena::with_capacity(5_242_880);
        let bit_vec: BitVec = BitVec::with_capacity_in(&arena, 100);
